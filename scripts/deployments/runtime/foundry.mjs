 ...
 */
export function runForgeScript({ target, rpcUrl, broadcast, verify, etherscanApiKey, env }) {
  const args = ['script', target, '--rpc-url', rpcUrl, '--via-ir'];
  if (broadcast) {
    args.push('--broadcast');
    if (verify) args.push('--verify', '--etherscan-api-key', etherscanApiKey);
  }
