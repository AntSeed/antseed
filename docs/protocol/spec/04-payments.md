...
`AntseedWashTradingRegistry` accepts two pinned RISC Zero proof formats: closed-loop and reciprocal. Closed-loop proofs authenticate positive funding of at least three buyers before selected seller settlements totaling at least 1,000 USDC, followed by seller-outward closure. Reciprocal proofs authenticate at least 100 bidirectional settlements with minimum directional count, volume, and exact 80% volume-balance thresholds.

The proofs use authenticated receipt and transaction inclusion and do not require historical EIP-1186 state proofs. Their funding guarantee is intentionally narrow: positive funding before the selected settlements, not “first funder” or funding before the buyer's first-ever channel. Receipt-only attribution requires the recovered transaction signer to equal the funder and rejects unsupported smart-account attribution.
...
The reward registry initially includes `AntseedHistoricalClaimsPolicy` and `AntseedWashTradingRewardPolicy` on both routes. The historical policy blocks all seller claims until the Base state oracle reports `historicalCoverageComplete()` and the owner records a nonzero signed proof-release digest with `finalizeBackfill`. This one-way release attests that the complete wash-trading proof batch was submitted. After release, unflagged sellers can claim while the wash-trading policy permanently blocks flagged sellers.
