...
Legacy billing wire payloads are rejected. Compatible local seller configurations are migrated before startup with conditions and additive components preserved; incompatible or inexact prices require explicit reconfiguration. Existing historical receipts and database columns are unchanged.
