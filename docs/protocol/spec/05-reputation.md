...
When a peer has no `reputationScore`, the value **0** is used (treated as unknown/unverified). Official routers use the buyer-computed trust score (`trust = washFlagged ? 0 : usage + power + identity`, weights 40 / 20 / 40) before falling back to the locally reported score. Lifetime `AntseedChannels` counters are still read for display and the local sybil warning but are not part of the score.
