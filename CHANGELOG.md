...
- Added `antseed verifier submit --publish-ipfs`, which pins one complete public Pinata evidence folder per model before broadcast, records retryable publication state in the submission ledger, and emits the resulting `ipfs://` URI on-chain with the canonical bundle hash. This changes the verification contract ABI and requires redeployment.
...
- Added canonical local proxy-observation evidence containing reference data, pinned request/response hashes, verified `ResponseAuth`, cost telemetry, match vectors, statistical power, and verdicts. Evidence now uses a model-first epoch layout: each model stores its HTML report at the model root, reference integrity once under `references/<referenceId>`, each verification execution under `audits/<runId>`, and each seller pack under `sellers/<peerId>`. Run-level probe consensus links every authenticated answer directly to its seller exchange. It contains no payment evidence, and workflow state uses JSON artifacts rather than verification database tables.
- Added per-reference-point seller voting to verifier consensus evidence. Every scoreable answer backed by verified ResponseAuth votes regardless of the seller's final `SAME`, `DIFF`, or `UNDETERMINED` model-level verdict, ties confirm the reference, and model bundles commit the canonical consensus hash and vote summary on-chain through the existing evidence hash.
- Expanded each consensus reference point into a readable question-level evidence story with the OpenRouter source and answer, the physical range versus derived tolerance interval, counts and peer addresses for `CONFIRMED`, `REJECTED`, `NO_RESPONSE`, and `EXCLUDED`, the global majority decision, ResponseAuth signatures and hashes, and portable relative links to raw signed exchange files. Copied evidence ZIPs can now be inspected without links pointing back to the verifier's original machine.
