...
- OpenAI Responses sellers now preserve explicit reasoning effort across Chat Completions and Responses request adaptation, so `reasoning_effort: "none"` reaches the upstream as `reasoning: { effort: "none" }` instead of being silently removed and allowing the upstream to use a different default.
