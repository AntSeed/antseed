...
- Routing: one fixed-model/router selection replaces plugin model-name activation and separate saved mode flags. Request overrides and persisted session/chat selection control classification while concrete requests stay fixed by default. The generic classifier returns `{ "serviceId": "...", "peerId"?: "..." }`.
...
- Added a generic network-router adapter bundled with the CLI, using a versioned provider contract and existing automatic payment infrastructure. Providers do not need a custom buyer plugin. Includes response validation, decision reuse, an offline compatibility checker, and local-chain billing fixtures. The publishable `@antseed/router-classifier` package is outside the separate plugin auto-update catalog. No new channel-storage migration is included. See `plugins/router-classifier/README.md`.
