...
- Added a private classifier-router example that selects one eligible model through a host-authorized service and leaves seller selection to AntSeed, with response validation, decision reuse, and runnable local-chain billing fixtures. No new channel-storage migration is included. See `plugins/router-classifier/README.md`.
