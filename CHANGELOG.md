...
- Added local `antseed verifier benchmark run|report|attest` workflows for production observation. A benchmark takes one discovery snapshot, prepares and self-tests one shared KBF reference, sends the identical probe set to every eligible live peer, enforces a hard concurrent USDC spend cap, persists resumable batch evidence, exports every participant as table/JSON/CSV, and performs no registry write until explicitly selected completed audit IDs are passed to `benchmark attest`. Observe-only runs can now use an authenticated localhost buyer-proxy transport, reusing the Desktop wallet's funded production channels without exposing its private key while preserving verified `ResponseAuth`, finalized payment evidence, actual charges, and the proxy identity in canonical audit evidence.
- Added `antseed verifier start` as a paid local-verifier daemon backed by a normal buyer-role `AntseedNode`. The default `--mode attest` discovers seller/service pairs, applies cooldown and retry limits, submits final attestations, and claims finalized rewards. The one-shot `--mode observe --once` route instead runs the shared-reference benchmark against one service, enforces an explicit USDC cap, prints the complete local analytics report, and performs no verifier-registry or reward operations. `verifier audit`, `verifier status`, `verifier claim`, and `verifier reference build` provide immediate audit, operations, and recovery tooling.
...
- Added canonical direct-audit evidence containing trusted-reference data, paid request/response hashes, verified `ResponseAuth` payloads, payment records, performance observations, match vectors, statistical power, verdict, and model share. Verification migration `005` creates `direct_audits` and `direct_audit_probes`, preserves reference and `ResponseAuth` storage, and removes relay-era audit scheduling and settlement tables.
- Added Venice-anchored adaptive KBF enrollment for `gpt-5.6-sol` using the existing `KbfReferenceV1` format. Sol generates candidates across 15 domains, `kimi-k3`, `gpt-5.6-luna`, and `sonnet-4.6` screen for distinguishing probes, and independent Sol enrollment/self-test passes retain stable answers with recomputable Clopper-Pearson/binomial evidence. The reference endpoint, optional AntSeed peer pin, model mappings, frozen query profile, timeout/retry budget, and trust classification are configurable and content-hashed without exposing API keys.
- Added bounded parallel KBF generation and enrollment with concurrent domains, model preflights, initial/contrast passes, stability passes, and position-aligned self-test batches. New verifier policy controls cap global and per-model concurrency, while serialized checkpoints, exact request budgeting, retry backoff, and adaptive throttling preserve restart safety against the Venice smoke peer.
- Replaced reusable reference pools with a persistent certified KBF probe catalog and audit-specific `KbfReferenceV1` materialization. Service rounds generate the maximum target shortage, fresh-test shared probes once, start each target at 100 unseen probes, and grow by ten up to 750 until statistical power reaches 90%. Probe exposure is permanent per agent/service but remains reusable across different agents.
- Added automatic cohort-aware benchmark sizing. `verifier benchmark run` and one-shot observe mode now default to `--probes auto`, divide the family-wide alpha across the frozen eligible cohort, grow the shared reference from 100 through 750 probes until the configured power target passes, persist the chosen alpha/power/coverage policy for resume and reports, and grade every target using those exact stored parameters. Numeric `--probes` values remain fixed-size overrides.
- Added stable readable and `--json` verifier status output with approval, epoch credits, pending rewards, direct-audit lifecycle, target identity, reference provenance, verdict, evidence, and transaction state.
