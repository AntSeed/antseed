...
- Changed buyer routing to exclude sellers carrying an active (unretracted) model-substitution flag for the requested service; retracted DIFFs downgrade to a history signal, `findPeer` and incremental background discovery now carry the same verification enrichment as full discovery, and delegation hosts are discoverable via their capability DHT topic.
