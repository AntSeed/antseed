...
- Added buyer-delegated probe execution to the verifier network so probe traffic originates from organic buyer identities instead of the publicly-whitelisted verifier wallet (which cheating sellers could classify and special-case). Verifiers with `verifier.delegation.enabled` announce a `verification.probe-delegation.v1` capability on the DHT and dispatch fully verifier-crafted probe jobs over a new delegation protocol (message types 0x90–0x94); opt-in buyers with `buyer.delegate.enabled` relay those jobs byte-for-byte over their ordinary paid buyer path after checking the verifier against the on-chain whitelist. Delegates are untrusted transport: the verifier re-verifies each seller-signed ResponseAuth against the exact request it crafted and the response body returned, so a delegate can drop a job but never alter or fabricate an observation.
- Added delegate voucher crediting and rewards: after an audit's attestations land, the verifier signs an EIP-712 `DelegateVoucher` per carrier (sent over the delegation channel, message type 0x94, persisted by the buyer in `delegate/vouchers.jsonl`), listed with the new `antseed delegate vouchers` command, and claimed with `antseed delegate claim` — run as the buyer's deposits operator, it claims each voucher's credits via `AntseedVerifierRegistry.claimDelegateCredits` and then collects any finalized delegate ANTS rewards. The contract enforces what the old verifier-submitted `creditDelegates` flow only promised off-chain: the caller must be the operator registered for the voucher's buyer in `AntseedDeposits` (resolved at claim time — the buyer hot wallet never receives funds), voucher credits per probe commitment are capped by the `probeCount` of the verifier's credited attestations on that commitment (so grants are backed by real, cooldown-limited, commit-reveal audit work), and per-verifier epoch grant caps still apply at claim time. `AntseedVerifierRewards` splits the verification emissions bucket into verifier and delegate pools by `delegateShareBps` (default 20%, applied only to epochs with delegate credits, frozen with the budget at first touch); operators collect via `claimDelegateReward`. `buyer.delegate.payoutAddress` config is removed — there is nothing to configure or assert anymore.
...
- Added a pluggable `ProbeSource` to `@antseed/fingerprints` with a compositional entity/attribute generator (hundreds of thousands of probes) as the verifier daemon's default, demoting the static probe bank to a test/bootstrap fixture, plus per-service probe rotation so probes revealed by past audits are not reused. New `verifier` config fields: `probeSource` (`"compositional"` | `"bank"`), `probeRotationHistory`, `probeLogDir`.
...
- Changed delegate vouchers to always carry a canonical 65-byte signature: compact EIP-2098 signatures are normalized at decode (and malleable high-s forms rejected), so a voucher that passes the delegate's checks is always claimable on-chain rather than silently reverting and forfeiting the delegate's share of the emissions bucket. Verifier and delegate reward claiming is now fault-isolated per epoch, so one reverting epoch no longer blocks every later one.
