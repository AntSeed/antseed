...
- `@antseed/router-levanto`: a buyer-only router (`levanto-router`) calling the user's selected routing-service peer at `/_antseed/levanto-route`. Levanto hosts its own seller integration; no seller adapter or backend API credentials are included. Live generic string-enum preferences replace startup-only CQT configuration. Native inference cache observations feed Levanto's bounded, expiring cache estimates. Mixed rankings retain eligible recommendations in order instead of rejecting the entire response. Day passes, new reasoning capabilities, and desktop controls are not included.
- Shared unit billing: add opt-in `completed_requests` measurements and version-2 usage reports using the existing unit calculator and payment channels. Replace unreleased `fixedFeeServices` with independent `serviceExecution` and `serviceUnitBillingModels` configuration. API formats such as `levanto-routing` and `typesafe-systemone` do not select billing units; the billing model does. Health probes depend on execution type, not price. Existing image v1 reports, token billing and signed v12 discovery remain compatible; completed-request buyers and sellers must upgrade together. Requests do not send a unit-price header. Buyers retain their selected advertised price and authorize only independently accepted responses, with price, reserve and duplicate-charge checks; seller price changes can cause payment disagreement after execution.
