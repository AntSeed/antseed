...
- Website: added a live LLM API cost calculator that joins current OpenRouter model rates with active AntSeed seller offers, recalculates the cheapest seller for the visitor's input/output token mix, and clearly reports when OpenRouter is the cheaper option.
