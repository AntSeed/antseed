...
- Chat Completions adapters for Responses clients now expose an explicit `final_answer` tool. Models that omit it keep the response marked non-final, so Codex continues after interim text without heuristic text matching; models that call it end the turn normally.
