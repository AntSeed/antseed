...
- Added `antseed video create/status/list/cancel/download`, including buyer quote safety caps, resumable Range downloads, SHA-256 verification, and atomic output rename.
- Added single-charge video billing: buyers authorize the full quote before submission, and sellers earn it at upstream acceptance. Removed split settings and delivery-receipt payments while retaining verified downloads, quote caps, seller setup, discovery display, and diagnostics. Buyers bear post-acceptance failure/non-delivery risk; this is not escrow, and automatic refunds remain out of scope. Legacy split settings fail explicitly and buyers/sellers must upgrade together.
