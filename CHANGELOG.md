...
- Contracts: added `AntseedLegacySellerClaimPolicy`, a stateless claim policy that unfreezes the deployed `AntseedSellerRewardsPool` after the M001 cutover. It re-derives each seller's cumulative locked ANTS from legacy EmissionsV2/V1 state, releases a configurable share (default 1538 bps ≈ 10/65, optionally linearly vested), and returns zero for sellers flagged by a pluggable wash-trading source (`isProvenWashTrader`) or by the policy owner. The M001 cutover installs it on the pool (from the pool owner key) right after pinning the pool's registry facade, so legacy claims open the moment the flip lands.
