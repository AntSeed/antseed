...
- Seller startup now warns when `seller.publicAddress` is missing or non-public, and `antseed seller doctor` classifies and locally probes the announced endpoint with explicit NAT guidance. The transport documentation now covers port forwarding, firewalls, VPS/reverse-tunnel deployments, and is available at `/docs/protocol/transport` instead of the broken route.
