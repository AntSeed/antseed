...
- Added per-connected-application model selection in Desktop VPR. Codex and OpenCode can use In-app selection or choose any VPR catalog model; both receive the user's favorite and recommended AntSeed models in their native model pickers while In-app selection is active, and otherwise expose a single AntSeed VPR model. Other connected apps can independently choose any catalog model. Explicit app models use Best seller routing with failover, and changes apply only to new chats while existing chats retain their captured route.
...
- Fixed Codex losing its local terminal and filesystem tools when connected through Desktop VPR. Codex now uses a dedicated HTTP-only AntSeed provider with compatible tool capabilities while routing through the buyer endpoint, with hosted search and request compression disabled so the buyer can safely transform requests and apply the selected model route.
