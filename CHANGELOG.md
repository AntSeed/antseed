...
- Desktop / download proxy: installs can now be attributed to the website session and campaign that produced them. `download.antseed.com` stamps a signed attribution token into the served installer filename (`AntSeed-VPR-Setup-<version>.a-<token>.exe`); the Windows installer records its own filename beside the app and sends an `installer_started` beacon, and AppImage reads it from `$APPIMAGE`. The desktop app then reports coarse post-install milestones to the proxy — `app_first_opened`, `app_onboarded` (setup done and network joined), `app_activated` (first of: tool connected, first chat, deposit, routing change, API config copied, plugin installed; with the kind), `tool_connected` (which app), `first_chat_started`, `deposit_completed` — which the proxy forwards to GA4 under the download's original client id and session. Reports carry only the token, a random per-install id, and the same buckets as PostHog telemetry; they follow the telemetry kill switches and the in-app opt-out. Installs that cannot carry the stamp (macOS, renamed files) are matched server-side: the proxy keeps a 48-hour Workers KV record per attributed download keyed by platform and an HMAC of the client IP, and accepts a match only when exactly one download fits; every milestone carries `attribution_method` (`token`, `match`, `none`) so reports can weight exact and probabilistic attribution. An affiliate code on the download URL (`?ref=`) travels in the token and the match record and is forwarded as `ref`. Download events now also record the public site the link was clicked on (`referrer_host`, allowlisted hosts only, private hosts reported as `other`) and any `utm_*` tags the website carried onto the link (`link_source` / `link_medium` / `link_campaign`), both read server-side so downloads from browsers that block GA still get a source; the website remembers the landing page's tags for the tab session and adds them to proxy links. New proxy routes: `POST /app-events` and `GET /i?f=<filename>`; new proxy secret `ATTRIBUTION_SECRET` and optional KV binding `ATTRIBUTION_KV` (unset disables stamping, verification and matching respectively). Replaces the PostHog → GA4 webhook, whose events could not be attributed.
