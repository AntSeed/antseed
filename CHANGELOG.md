...
- Shared unit billing: add opt-in `completed_requests` measurements and version-2 usage reports using the existing unit calculator and payment channels. API format and pricing use ordinary provider configuration; the Levanto buyer plugin validates routing payloads, while providers validate their own APIs. No generic execution map or contract/price headers are added. Buyers retain price, acceptance, reserve and duplicate-charge checks. Routing uses ordinary request dispatch without a separate seller queue, busy guard or execution replay cache. Existing image v1 reports, token billing and signed v12 discovery remain compatible. Completed-request buyers and sellers must upgrade together; seller price changes or invalid successful provider responses can cause payment disagreement after execution.
