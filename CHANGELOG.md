...
- Contracts: the wash-trading registry now pins a concrete `SP1VerifierGroth16` deployment by `VERIFIER_HASH()` (constructor argument `expectedVerifierHash`, exposed as `verifierHash`) and rejects the routable `SP1VerifierGateway`; `AntseedSparseBlockhashStore` treats Chainlink `BlockhashStore` "blockhash not found" reverts as an unknown block (`bytes32(0)`) so the contract's own `MissingAnchor`/`NonCanonicalBlock` errors surface.
- Contracts: added a resumable sparse blockhash backfill with a digest-pinned local header cache, plus an Anvil E2E that stages, authenticates, and finalizes a development seller proof.
...
- Contracts: isolated persistent sparse-blockhash frontiers by submitter so another account cannot poison a predictable backfill session.
