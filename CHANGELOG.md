...
- Native Runway/Veo video API relays for seller-operated services, video discovery, acceptance-based unit billing, and persistent buyer-side job-to-seller/provider routing. Native request bodies and extension fields are preserved byte-for-byte, independent of chat model aliases and pins; unavailable recorded providers fail rather than silently switching providers. The seller node records which buyer created each job and returns 404 to any other buyer who polls or cancels it. Video creates carry an `x-antseed-idempotency-key`; if an acceptance response is lost, the buyer proxy retries the same seller with the same key and the seller replays the stored acceptance with no second job or charge. Veo billing reads Gemini's `numberOfVideos` and accepts string durations; Runway `duration: "auto"` requires per-generation pricing. Sellers own job execution and result delivery; no AntSeed job workers, artifact cache, or automatic refund guarantee.
