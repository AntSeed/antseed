...
- AI VPN: reconcile buyer channels against finalized on-chain state in the background, preserving history while retiring verified closed channels and expired, unused openings whose authorization matches the configured deployment. Credits and Activity now share the same verified-channel count and list unresolved records separately instead of presenting them as active or offering unverified close actions.
