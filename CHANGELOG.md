...
- The buyer proxy now answers `GET /v1/models` locally with a network-wide model list aggregated from discovered peers — no peer pin required. Cosmetic variants and conservative aliases such as `claude-opus-5`, `opus-5`, and `opus5` merge into one model and are exposed in `aliases`; numeric-version variants such as `gpt-5.6-sol`, `gpt-56-sol`, and `openai-gpt-56-sol` also merge across established model families, while materially different suffixes such as `pro`, `fast`, and `web` remain separate. Every `peers` offer retains its actual advertised `serviceId` for explicit routing and now exposes its resolved protocol, announced protocols, categories, pricing, reputation, context and output limits, modalities, reasoning, tool-use, structured-output, and supported-parameter capabilities. Model-level capability fields are conservative guarantees across every offer, while `capability_coverage` distinguishes unknown metadata and `supported_protocols` summarizes the network-wide protocol union. Peer offers use the desktop VPR's reputation ordering: trust first, then reputation, with unknown scores last. Requests that provide only a model name now select the highest-reputation compatible peer allowed by buyer policy, skip peers in an active PR #750 cooldown when another offer is ready, rewrite to the seller's advertised service ID, and fall back through lower-ranked peers after peer-attributed retryable failures. Desktop Auto routes, Telegram model selections, automatic in-app chats, and connected-app `antseed` aliases now persist only the selected model, so live peer selection and fallback remain active; a seller chosen explicitly in the desktop and `<peerId>@<serviceId>` requests remain single-peer. Buyer-local failures and started streams never fail over. `?type=text` / `?type=images` filters the list, and `GET /v1/models/<id>` looks up a single model across the network.
