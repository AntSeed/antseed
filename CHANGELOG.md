...
- Added buyer-delegated probe execution to the verifier network so probe traffic originates from organic buyer identities instead of the publicly-whitelisted verifier wallet (which cheating sellers could classify and special-case). Verifiers with `verifier.delegation.enabled` announce a `verification.probe-delegation.v1` capability on the DHT and dispatch fully verifier-crafted probe jobs over a new delegation protocol (message types 0x90–0x93); opt-in buyers with `buyer.delegate.enabled` relay those jobs byte-for-byte over their ordinary paid buyer path after checking the verifier against the on-chain whitelist. Delegates are untrusted transport: the verifier re-verifies each seller-signed ResponseAuth against the exact request it crafted and the response body returned, so a delegate can drop a job but never alter or fabricate an observation.
- Added on-chain delegate crediting and rewards: `AntseedVerifierRegistry.creditDelegates` lets verifiers credit the operator payout addresses that carried verified probe jobs (aggregated once per audit round, capped per verifier per epoch), and `AntseedVerifierRewards` now splits the verification emissions bucket into verifier and delegate pools by `delegateShareBps` (default 20%, applied only to epochs with delegate credits, frozen with the budget at first touch). Delegates claim via `claimDelegateReward`; credits always target the operator registered for the delegate buyer in `AntseedDeposits` — resolved automatically on the buyer side and verified on-chain by the verifier before accepting a delegate registration — never the buyer hot wallet.
