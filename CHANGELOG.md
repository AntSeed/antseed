...
- Discovery metadata v13 adds an `operation` image unit-billing match key so sellers can advertise different `image_generation` and `image_edit` prices. Buyers derive the operation from the original OpenAI Images path on both sides of payment verification. This is a buyer-first metadata rollout: v13 buyers remain compatible with v10-v12 sellers, while older buyers reject v13 sellers cleanly.
- Venice image edits now reject unsupported multipart fields and `response_format=url` before contacting the upstream API instead of silently dropping parameters or returning non-portable `data:` URLs. Successful edits always return OpenAI-compatible `b64_json` data, and seller startup warns when an `imageEditModel` has no explicit `openai-images` billing model.
- `@antseed/provider-openai` now supports native Venice image editing. Sellers pair an advertised generation service with an explicit `imageEditModel`; `/v1/images/edits` multipart requests are translated to Venice's `/api/v1/image/edit`, edit-specific models are selected without changing the public service ID, raw image responses are converted to the OpenAI Images shape, and Venice errors retain their upstream status. Only paired services advertise image input and moderation support; generation-only Venice services advertise text input and reject edits before any invalid upstream request. The CLI adds `antseed config seller add-service --image-edit-model` and serializes the durable pairing for the provider.
