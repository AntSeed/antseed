...
- Added the split `AntseedVerificationPointsPolicy` integration to Node and CLI configuration, plus the verification-only `ConfigureVerification.s.sol` deployment workflow. It transfers the preconfigured verification emission bucket from its temporary wallet to `AntseedVerification` and leaves the policy adapter unregistered for shadow mode by default, without depending on wash-trading deployment.
