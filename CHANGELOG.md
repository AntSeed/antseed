...
- Buyer payments: add shared per-call accounting, request-scoped billing and attribution, synchronous response-acceptance hooks, cancellation cleanup, and serialized payment updates for concurrent requests to one seller. Preserve existing reserve top-up policy and the released database schema. This is PR 3/4 of the routing stack; buyer activation remains in PR 4/4, with no temporary standalone-execution guards.
