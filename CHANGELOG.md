...
- Added `antseed verifier run <model> [--benchmark]`, which discovers every live peer advertising the model, sends the same 100 paid KBF probes, verifies seller-signed `ResponseAuth`, writes canonical per-peer evidence and one local JSON run summary, and submits successful results on-chain unless `--benchmark` is set. Benchmark mode requires buyer payment readiness but no verifier approval, verifier contracts, or attestation gas. `antseed verifier claim` remains available for rewards.
...
- Added canonical local verification evidence containing fixed-reference data, paid request/response hashes, verified `ResponseAuth` payloads, payment records, match vectors, statistical power, and verdicts. Workflow and benchmark state use JSON artifacts rather than verification database tables.
...
- Added one reusable 100-probe JSON reference per model. `verifier run` generates and certifies it from the configured trusted endpoint only when the fixed artifact is missing or invalid, then reuses the same questions for every peer and later run.
...
- Changed verifier configuration to only accept evidence/reference directories, request timeout, a required total-run USDC cap, and the trusted reference endpoint/model mapping. Removed daemon scheduling, adaptive sizing, catalog rotation, benchmark persistence, and legacy upstream options.
