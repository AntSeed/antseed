...
- Added a consolidated `AntseedVerification` API with direct final attestations, compact metric events, hash-committed evidence, latest per-service state, seller points penalties controlled by the latest conclusive audit, and an owner-configurable 100-credit verifier epoch cap. Valid attestations beyond the cap still update verification state and penalties; full history is derived from events.
