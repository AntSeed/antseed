...
- Desktop Price + Trust settings now persist to `buyer.routingPreferences`, and the running buyer proxy hot-reloads them so `/v1/models` peer order and model-only request dispatch use the same policy. Automatic text and image chats remain model-only for live fallback, while explicitly selected sellers remain pinned.
