...
- `@antseed/node` buyers can now hold payment authority in a signer separate from the libp2p identity, via a new optional `payments.buyerSigner` (`NodeConfig`). By default the node still pays from its libp2p identity wallet, which requires an exportable secp256k1 private key; setting `buyerSigner` makes that signer the on-chain buyer instead — it signs the EIP-712 `ReserveAuth` and `SpendingAuth`, and its address is recorded as `buyer` on channels and free-usage sessions — while the libp2p identity keeps the peerId and signed peer metadata. This lets buyers whose keys are non-exportable by construction — threshold signature wallets, MPC custody, HSMs, and custodial platforms paying on a user's behalf — use the network. A new `node.buyerAddress` getter reports which address the node pays from. Note `deposit()` and `withdraw()` still submit transactions through this signer and so need gas and transaction signing; a signTypedData-only signer should be funded out-of-band (`AntseedDeposits.deposit(buyer, amount)` accepts any sender) and use the meta-transactional `setOperator` to authorize withdrawals.
