...
- ANTS dashboard: the Network page's emissions panel now shows where the epoch emission goes per the deployed contracts: each gate bucket's fixed ceiling next to what the seller-pools and usage contracts actually allocate this epoch, the live figure and target that set each dynamic share (active stake vs. the stake target, settled volume vs. the volume target), and the unallocated remainder that is burned or reserved. The "editable" minter column is gone; `GET /api/emissions` drops `editable` and adds `epochVolumeUsdc`.
- ANTS dashboard: a position that has not reached its activation epoch is no longer labelled as having a pending change; its actions explain that lock changes are possible from the activation epoch. Split, merge and move sources that close next epoch are no longer counted as stake pending activation.
- Standalone ANTS dashboard: browser builds skip public gateways that reject cross-origin requests, poll the wallet RPC every 30s instead of 4s, and retry page loads after a failed fetch. Public Base gateways still rate-limit a connected wallet's reads within minutes; deploy with `VITE_ANTS_RPC_URL` set to a dedicated, origin-restricted endpoint.
