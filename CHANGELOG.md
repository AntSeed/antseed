...
- CLI: classifier calls from concurrent conversations queue per seller instead of conflicting over a payment grant. Waiting is bounded and cancellable, and does not extend request deadlines or serialize inference. Conversation bookkeeping coalesces writes over 250 ms and flushes on shutdown.
...
- Added a privately installable classifier-router plugin that selects one eligible model through a host-authorized service and leaves seller selection to AntSeed, with response validation, decision reuse, and runnable local-chain billing fixtures. The `classifier` alias loads a local installation without npm installation or updates. The package remains private and unpublished. No new channel-storage migration is included. See `plugins/router-classifier/README.md`.
