...
- Routing: extend the in-review Levanto v1 interface (no version negotiation) with router-supplied supported-model catalogs. Router adapters may implement optional `getCatalog()`; the Levanto adapter fetches `GET /_antseed/route/catalog` from the router's API (`LEVANTO_ROUTING_PEER_URL`, default the routing peer's host on port 8787). Peer metadata is unchanged. Catalogs are content-hash revisioned, cached for 60 seconds, restrict candidates before paid routing, and supply the preference schema (string enums with optional titles). Requests send generic `preferences`, `catalogRevision` and exact `allowedCandidates`; responses must echo the revision and include `provider`. Catalog changes (409), empty intersections (422) and unpaid work (402) fail without inference. Router selections accept an optional exact `allowedModels` list. `GET /_antseed/routing-services` lists completed-request routing offers with prices and catalogs. Responses API `input` is accepted for routing. The buyer no longer double-counts a response's cost when NeedAuth already counted it. External Levanto routers must add the catalog API before release.
- Buyer routing: register protocol-specific adapters through `ModelRouterRegistry` and `createLocalRouter(config, adapters)`. The selected service's advertised protocol selects its adapter and preference schema; unknown or ambiguous protocols fail closed. Levanto remains registered by default. Billing, metadata wire formats and the desktop picker are unchanged.

- Levanto recommendations use the remote `POST /_antseed/levanto-route` endpoint; external sellers must configure the matching service path. The local `/_antseed/route` control API uses the selection-only format described below.

- `@antseed/router-levanto`: a buyer-only adapter included by the existing `local` router, calling the user's selected routing-service peer at `/_antseed/levanto-route`. Select a model or a routing service without switching router plugins; model mode remains the default. Routing purchases use the selected advertised price without a separate routing-fee setting or user-configured price ceiling; each purchase remains bounded to its price snapshot. Levanto hosts its own seller integration; no seller adapter or backend API credentials are included. Live generic string-enum preferences replace startup-only CQT configuration. Native inference cache observations feed Levanto's bounded, expiring cache estimates. Mixed rankings retain eligible recommendations in order instead of rejecting the entire response. Day passes, new reasoning capabilities, and desktop controls are not included.
- Shared unit billing: add opt-in `completed_requests` alongside `output_images` in the existing version-1 `priceUsd` component and usage-report formats, reusing the unit calculator and payment channels. Prices are advertised directly in `serviceUnitBillingModels` with signed metadata v12; no special `offerings` entry is needed. Existing image/token-only sellers remain compatible with old buyers, but old buyers reject any announcement containing the new unit, including mixed sellers. Run new-unit services on separate peers initially. The Levanto plugin validates routing payloads; providers validate their own APIs. Buyers retain price, acceptance, reserve and duplicate-charge checks. No generic execution map, contract/price headers, seller queue, busy guard or execution replay cache is added. Completed-request support is identified by the signed billing model, without a separate discovery or connection capability flag; incompatible buyers are not rejected by an upfront capability check. Completed-request peers must upgrade together; seller price changes or invalid successful responses can cause payment disagreement after execution.
- CLI routing selection: select a model or an exact routing-service target through the local route endpoint and restore it from saved state. No startup configuration option is added; without saved selection the buyer starts in model mode with no default model. Existing price/trust policy hot reload is unchanged. Router mode ignores stale default/connected-app models while preserving explicit user pins. The control API accepts `{ selection }` and returns `{ ok, selection }`; saved state uses `selection`, with a one-time conversion of valid legacy `defaultRoutedModel` values when `selection` is missing. Existing selections, including cleared defaults, take precedence. Legacy `model` API responses are not restored. Desktop/VPR model-only callers require a follow-up update. Selection changes affect subsequent requests without cancelling in-flight work. Preference schemas are defined in the installed model-router adapters; no HTTP metadata endpoint is added.
- CLI conversation routing: persist an exact router-service target and validated preferences per chat through the conversation update endpoint. Unconfigured chats inherit buyer defaults; clearing an override restores inheritance. Child requests follow their parent chat's selection; changes apply to subsequent requests without cancelling in-flight work. Explicit model/seller pins replace router overrides; no new desktop controls are included.
...
- CLI paid routing: include signed routing-service charges in the originating conversation's spend, even when inference fails or authorization arrives later. Register purchases before dispatch and include their reported input, cached-input, and output tokens without adding extra conversation request counts. Payment wire formats, image billing, and ordinary request accounting are unchanged.
