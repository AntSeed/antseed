...
- The testing-only `openai-responses` provider now treats existing aliased service IDs ending in `-fast` as OpenAI Fast processing and forces `service_tier: "priority"`, without introducing a separate Fast configuration path.
