...
- Opt-in purchase-on-use router access with exact discovered terms, generic desktop/CLI activation and pause controls, and transactionally persisted approvals and authorizations. Missing prices fail closed; changed terms require reapproval, idle periods are not charged, and restart/channel replacement cannot reset the buyer's paid-period clock. See `docs/router-shipping-handoff.md` for scope and external seller rollout requirements.
- Simplified routing to the initial model choice per conversation, with same-model peer failover and no reclassification on later turns, compaction, or refresh headers. Explicit models bypass classification. Router settings are plugin-owned; removed shared day-pass consent/configuration, the universal `cqt` preference, and the mandatory Levanto catalog entry.

- SDK and CLI support fixed per-call routing fees alongside token and day-pass billing. Per-call routing requires a plugin parser and host eligibility validation before authorizing payment; invalid classifications and route reuse without a new classifier call do not authorize a new fee. See `docs/router-per-call-billing.md` for configuration and the fail-closed behavior of a seller that claims payment for invalid output.
