...
- Levanto recommendations use the remote `POST /_antseed/levanto-route` endpoint; external sellers must configure the matching service path. The generic local `/_antseed/route` selection API is unchanged.
...
- CLI routing selection: select a model or an exact routing-service target with `buyer.selection` or the local route endpoint. Router mode ignores stale default/connected-app models while preserving explicit user pins. Selection changes invalidate cached decisions and cancel in-flight routed requests. A metadata endpoint exposes the installed router's generic preference schema for future UI controls.
- CLI conversation routing: persist an exact router-service target and validated preferences per chat through the conversation update endpoint. Unconfigured chats inherit buyer defaults; clearing an override restores inheritance. Child requests follow their parent chat's selection, and changing one chat cancels only that chat's routed requests. Explicit model/seller pins replace router overrides; no new desktop controls are included.
