...
- Codex also continues when chat-only models end a step with short interim progress text but no tool call. Interim phrasing such as "Let me grab the exact lines first" is marked non-final so the client requests the next step, while genuine final answers still end the turn normally.
