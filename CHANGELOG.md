...
- ANTS dashboard: require exactly one of `--local-address` or `--address <address>` to launch. Plain `antseed ants` exits with an error explaining both options, without loading an identity or starting a server. Both flags keep browser-wallet signing. Seller/staking actions require the selected account's wallet; buyer actions require its current deposits operator. Selection stays fixed across wallet changes; invalid addresses, mismatched wallets, and missing local identities produce explicit errors. Explicit external accounts cannot use the local identity's authorization flow. CLI subcommands and `--help` do not require these dashboard flags.
