...
- The buyer proxy now answers `GET /v1/models` locally with a network-wide model list aggregated from discovered peers — no peer pin required. Each entry carries the model's `type` (`text` or `image`) and a `peers` array with every seller serving it (peer id, provider, protocols, token pricing, per-image price range), sorted by reputation. `?type=text` / `?type=images` filters the list, and `GET /v1/models/<id>` looks up a single model across the network. Previously the request was forwarded to the pinned peer and returned only that seller's service ids.
- Desktop: the Telegram bot can now send images. A new `/image <prompt>` command generates an image through the best-ranked image seller within the buyer routing policy and delivers it as a photo (large or exotic formats fall back to documents), generated images in chat replies are delivered instead of dropped, `/stop` cancels a running image generation, and image-only models no longer appear in the bot's text `/model` picker fallback.
