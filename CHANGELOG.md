...
- Verified GitHub and domain identities now carry an optional third-party rank in a new pluggable provider registry. Evidence version 3 records a `thirdPartyRank` produced by the first enabled provider that matches the claim kind; the defaults are ghfind for GitHub accounts (up to 30 points toward the 70-point ceiling, organization logins unsupported) and Tranco for domains (log-scaled traffic-rank credit up to 12 points toward the 12-point ceiling, unlisted domains earning nothing). Both are independent sources of recognition applied within the existing per-identity caps, exposed as provider-level `providerScore` / `providerPoints` and a rolled-up `externalProviderScore` in routing breakdowns, and both run buyer-locally in the background verification queue with the same HTTPS hardening and seven-day freshness rules. Each provider is independently opt-out via `ThirdPartyRankingConfig.disabled` or the `ANTSEED_RANK_<NAME>` env switch (`ANTSEED_RANK_GHFIND=0`, `ANTSEED_RANK_TRANCO=off`), and the registry itself can be replaced through `ThirdPartyRankingConfig.providers` or `NodeConfig.externalHistory.ranking`.

- Verified GitHub identities now receive age-gated, diminishing follower credit worth up to 20 points within the existing 70-point public-history ceiling. Counts come from the public account API, refresh without extra requests, remain distinct from repository stars, and appear separately in reputation breakdowns and desktop tooltips. Version 1 evidence remains readable without follower credit; new evidence uses version 2.

- Buyers now bootstrap provider reputation from independently verified public GitHub portfolios and exact-match authoritative RDAP domain registration history, without brand allowlists or required API credentials. Versioned, buyer-local evidence can contribute up to 70 routing points while preserving failure penalties, cooldowns, and existing price policies; duplicate identities and parent-domain ages cannot stack credit. CLI diagnostics and desktop reputation tooltips distinguish public-history credit from chain reputation, and external fetches enforce public-address DNS pinning, redirect rejection, timeouts, response limits, and bounded caching. Public history remains a heuristic rather than proof of service quality.
