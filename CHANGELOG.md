...
- Fixed Desktop image chats dropping an explicitly selected seller when moving from a model page into chat. Image follow-ups now use only sellers that advertise image input support, generation-only pinned sellers show an actionable error instead of receiving a guaranteed-failing `/v1/images/edits` request, and payment progress no longer replaces the image shimmer with the text loader.
