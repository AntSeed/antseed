...
- Routing: recommendations can choose `inference.reasoningEffort`. Missing seller effort metadata allows best-effort choices without seller configuration changes; explicit effort lists, `reasoning: false`, and target-protocol limits remain enforced. Backend support is not guaranteed, and no automatic retry without reasoning is added. Router choices override client effort; `none` disables reasoning, while omission preserves compatible client settings. Non-reasoning services do not inherit client reasoning controls. Existing continuation reuse retains each ranked choice's effort and revalidates capabilities; no new call-avoidance controls are added. The same signed metadata v14 extension carries routing descriptors and optional accepted effort labels; announcements without either keep their existing version selection.

- Routing: structured requests can include bounded, in-memory inference usage history for eligible conversation offers. Router-scoped IDs support deduplication without exposing session IDs; missing cache counts remain unknown, not zero. Observations accumulate during continuation reuse and accompany the next routing call. No extra reporting requests, persistence, prediction algorithm, or decision-reuse controls are introduced.

- Routing: network services use `antseed-routing` at `/v1/route`, with signed per-service preference schemas in metadata v14. CLI/config preserves typed values in `buyer.selection`; `antseed buyer router describe --json` inspects schema and effective values. The chat-classifier adapter is retired with migration guidance. Older buyers cannot consume v14 announcements; ordinary announcements retain their previous version selection.
- Routing: one fixed-model/router selection replaces plugin model-name activation and separate saved mode flags. Request overrides and persisted session/chat selection control classification while concrete requests stay fixed by default. Network routing returns a ranked `recommendations` list containing `serviceId` and optional `peerId` per entry. Retryable inference failures advance to the next eligible listed model without another routing call; cancellation, buyer faults, payment-required responses and started streams do not fail over. Continuations retain the remaining ranked choices. Invalid, duplicate or partially ineligible lists are not accepted as successful per-call routing results.
- Discovery: routing services must advertise `routing: true` and are excluded from inference listings/candidates. Metadata v13 carries this capability; ordinary announcements remain v12 and v10–v12 wire baselines are unchanged.
...
- CLI/SDK: classification uses ordinary parallel seller requests, including multiple chats and an open inference stream on the same peer. Request-specific billing snapshots, cancellation, response acceptance, and pending costs replace classifier-only payment grants and the peer request gate. Shared channel payment updates remain serialized, without holding the queue during response, acknowledgement, or balance-RPC waits. Conversation bookkeeping coalesces writes over 250 ms and flushes on shutdown.
- Payments: keep the existing top-up behavior on balance-RPC failures in this routing change; reserve diagnostics are debug-only. RPC-failure hardening remains a separate payment follow-up.

- CLI: use one `buyer.selection` value for a fixed model or router, with an optional network service target. Remove redundant router-mode/enablement configuration, classifier-specific limits, and default-model error fallback. Preserve explicit pins and invalidate automatic routing context when selection changes. Buyer provider/service price exceptions remain deferred; existing global policy applies.
- Routing payments: recovery replays do not advance spending, optional reserve top-up failures preserve paid classifications, seller preflight accounts for `successful_requests`, and unit billing checks the requested protocol. Fixed-fee classification acceptance uses the shared request handler's optional response-validation hook.
...
- CLI: router configuration commands accept complete selection and namespaced settings objects from a fresh configuration. Boolean fields parse `true`/`false` correctly, existing string-valued payment limits retain their exact values, and invalid selections/settings are rejected before saving.
...
- Routers can recommend a model with automatic seller selection or an exact model/seller offer, with explicit ranked fallback across models. Continuations retain that intent separately from the dispatched seller; selecting a fixed model bypasses classification and selecting the router re-enables it. Fixed-fee classifier acceptance supports both recommendation forms under buyer policy.

- Added a generic network-router adapter in shared router infrastructure, bundled with the CLI, using a versioned provider contract and existing automatic payment infrastructure. Providers do not need a custom buyer plugin. Includes response validation, decision reuse, an offline compatibility checker, and local-chain billing fixtures. No new channel-storage migration is included. See `docs/router-network-integration.md` and `docs/protocol/templates/routing-provider`.
