...
- Codex conversations in the VPR now use Codex's own sidebar title when available. Existing chats whose injected plugin context was removed no longer fall back to raw session IDs, user-assigned VPR labels still take precedence, and other integrations retain their existing title behavior.
