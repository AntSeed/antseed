...
- Replaced single-seller verification submissions with `submitVerificationBundle`: each model transaction stores total audit cost, expected epoch, evidence hash, and requested credits once, then emits compact seller results containing agent ID, service hash, verdict, and model-share BPS. Probe count is removed from the ABI and events.
...
- Verifier credits now represent rounded-up whole dollars of accounted model-audit cost, remain capped at 100 credits per verifier per epoch, and continue to weight the existing pro-rata ANTS reward pool. Zero-cost bundles request zero credits, and bundles above the remaining cap still apply their seller results.
