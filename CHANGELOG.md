...
- The buyer proxy now answers `GET /v1/models` locally with a network-wide model list aggregated from discovered peers — no peer pin required. Cosmetic variants and conservative aliases such as `claude-opus-5`, `opus-5`, and `opus5` merge into one model and are exposed in `aliases`; numeric-version variants such as `gpt-5.6-sol`, `gpt-56-sol`, and `openai-gpt-56-sol` also merge across established model families, while materially different suffixes such as `pro`, `fast`, and `web` remain separate. Every `peers` offer retains its actual advertised `serviceId` for explicit routing. Peer offers expose their on-chain trust and reputation scores and use the desktop VPR's ordering: trust first, then reputation, with unknown scores last. Requests that provide only a model name now select the highest-reputation compatible peer allowed by buyer policy, skip peers in an active PR #750 cooldown when another offer is ready, rewrite to the seller's advertised service ID, and fall back through lower-ranked peers after peer-attributed retryable failures. Buyer-local failures and started streams never fail over; `<peerId>@<serviceId>` remains single-peer. Each entry carries the model's `type` (`text` or `image`) and seller offers with peer id, provider, protocols, token pricing, and per-image price range. `?type=text` / `?type=images` filters the list, and `GET /v1/models/<id>` looks up a single model across the network.
