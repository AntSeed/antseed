...
- CLI: new `antseed buyer activity` command — the terminal counterpart of the desktop's Activity view: lifetime tokens/spent/saved tiles (savings measured against OpenRouter retail prices across matched models), a per-day spending chart over 7/30/90 days built from channel snapshots, active channels with on-chain locked amounts (with channel IDs and close guidance), the deposits balance, and claimable ANTS emissions. Served by the running buyer connection; `--json` for scripts.
