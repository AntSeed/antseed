...
- The buyer proxy now answers `GET /v1/models` locally with a network-wide model list aggregated from discovered peers — no peer pin required. Cosmetic variants and conservative aliases such as `claude-opus-5` and `opus-5` merge into one model while every `peers` offer retains its actual advertised `serviceId` for routing. Each entry carries the model's `type` (`text` or `image`) and seller offers with peer id, provider, protocols, token pricing, and per-image price range, sorted by reputation. `?type=text` / `?type=images` filters the list, and `GET /v1/models/<id>` looks up a single model across the network. Previously the request was forwarded to the pinned peer and returned only that seller's service ids.
