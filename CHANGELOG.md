...
- The buyer's streaming-response duration cap is now configurable and its default was raised from 5 to 30 minutes. Streams longer than the cap were cancelled mid-response, which surfaced in OpenAI Responses clients (e.g. Codex CLI) as `stream disconnected before completion: stream closed before response.completed` on long agentic turns. Set `buyer.maxStreamDurationMs` in config.json or the `ANTSEED_BUYER_MAX_STREAM_DURATION_MS` environment variable to tune it.
