...
contract AntseedEscrow is EIP712, Pausable {
    ...
    // ─── Dynamic Buyer Credit Limits ───
    uint256 public BASE_CREDIT_LIMIT;              // default 10_000_000 (10 USDC)
    uint256 public PEER_INTERACTION_BONUS;         // default 5_000_000 (5 USDC per unique seller)
    uint256 public TIME_BONUS;                     // default 500_000 (0.50 USDC per day)
    uint256 public PROVEN_SESSION_BONUS;           // default 10_000_000 (10 USDC per proven session)
    uint256 public FEEDBACK_BONUS;                 // default 2_000_000 (2 USDC per feedback)
    uint256 public MAX_CREDIT_LIMIT;               // default 500_000_000 (500 USDC)
    ...
    struct BuyerAccount {
        ...
        uint256 firstSessionAt;      // timestamp of first session (for TIME_BONUS)
        uint256 provenBuyCount;      // number of proven sessions signed as buyer
        uint256 feedbackCount;       // number of on-chain feedbacks submitted
    }
    ...
    mapping(address => uint256) public creditLimitOverride;  // owner-set per-buyer override (0 = use formula)
...
**Implement:**
- `getBuyerCreditLimit(address buyer) → uint256` — computes dynamic limit:
  ```solidity
  function getBuyerCreditLimit(address buyer) public view returns (uint256) {
      uint256 override_ = creditLimitOverride[buyer];
      if (override_ > 0) return override_;

      BuyerAccount memory ba = buyers[buyer];
      uint256 daysSinceFirst = ba.firstSessionAt > 0
          ? (block.timestamp - ba.firstSessionAt) / 1 days
          : 0;

      uint256 limit = BASE_CREDIT_LIMIT
          + PEER_INTERACTION_BONUS * uniqueSellersCharged[buyer]
          + TIME_BONUS * daysSinceFirst
          + PROVEN_SESSION_BONUS * ba.provenBuyCount
          + FEEDBACK_BONUS * ba.feedbackCount;

      return limit > MAX_CREDIT_LIMIT ? MAX_CREDIT_LIMIT : limit;
  }
  ```
- `setCreditLimitOverride(address buyer, uint256 limit)` — onlyOwner, 0 = use formula
- `deposit(uint256 amount)` — transfers USDC, enforces MIN_BUYER_DEPOSIT on first deposit, enforces `balance + amount <= getBuyerCreditLimit(buyer)`, updates lastActivityAt
...
function setConstant(bytes32 key, uint256 value) external onlyOwner {
    ...
    else if (key == "BASE_CREDIT_LIMIT") BASE_CREDIT_LIMIT = value;
    else if (key == "PEER_INTERACTION_BONUS") PEER_INTERACTION_BONUS = value;
    else if (key == "TIME_BONUS") TIME_BONUS = value;
    else if (key == "PROVEN_SESSION_BONUS") PROVEN_SESSION_BONUS = value;
    else if (key == "FEEDBACK_BONUS") FEEDBACK_BONUS = value;
    else if (key == "MAX_CREDIT_LIMIT") MAX_CREDIT_LIMIT = value;
...
function setCreditLimitOverride(address buyer, uint256 limit) external onlyOwner {
    creditLimitOverride[buyer] = limit; // 0 = use formula
}
...
- **test_deposit_revert_creditLimitExceeded:** Deposit above credit limit reverts
- **test_deposit_subsequent:** Additional deposits within credit limit work
- **test_creditLimit_growsWithUsage:** After proven sessions and seller interactions, limit increases
- **test_creditLimit_override:** Owner sets override, buyer can deposit up to override amount
- **test_creditLimit_maxCap:** Limit never exceeds MAX_CREDIT_LIMIT
- **test_getBuyerCreditLimit:** Returns correct computed limit based on all factors
