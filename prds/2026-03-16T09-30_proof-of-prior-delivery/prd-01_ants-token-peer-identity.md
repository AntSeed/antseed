...
- [ ] `mint()` works even when transfers are disabled (minting is always allowed)
- [ ] `transfer()` reverts when `transfersEnabled == false`
- [ ] `transferFrom()` reverts when `transfersEnabled == false`
- [ ] `enableTransfers()` enables transfers permanently
- [ ] `enableTransfers()` reverts on second call (one-way toggle)
...
- **test_mint_worksWhenTransfersDisabled:** Minting succeeds even in phase 1
- **test_transfer_revert_transfersDisabled:** transfer() reverts before enableTransfers()
- **test_transferFrom_revert_transfersDisabled:** transferFrom() reverts before enableTransfers()
- **test_enableTransfers:** Owner enables, transfers work after
- **test_enableTransfers_revert_notOwner:** Non-owner reverts
- **test_enableTransfers_revert_alreadyEnabled:** Second call reverts (one-way toggle)
- **test_transfer_afterEnabled:** Standard ERC-20 transfer works after enableTransfers()
- **test_approve_transferFrom_afterEnabled:** Standard ERC-20 approve + transferFrom after enable
