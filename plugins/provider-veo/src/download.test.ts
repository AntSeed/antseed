...
const request = { requestId: 'download', method: 'GET', path: '/v1beta/models/veo/operations/task/videos/0:download', headers: { 'x-antseed-video-download': 'video-stream-v1', 'x-antseed-service': 'veo', 'x-goog-api-key': 'buyer-key' }, body: new Uint8Array() };
...
it.each([
  ...
  [() => file('video', { 'content-length': '4294967296' }), 413],
...
it('only advertises downloads for direct Google services', async () => {
  expect((await plugin.createProvider(config)).serviceCapabilities?.veo?.videoDownload).toBe('video-stream-v1');
