...
describe('provider-openai plugin', () => {
  ...
  it('advertises and rewrites aliased image services correctly', async () => {
    ...
    try {
      ...
      expect(provider.serviceUnitBillingModels?.['cover-art']?.['openai-images']).toEqual({ version: 2, components: [{ priceMicroUsdc: "0" }] });
  ...
  it('advertises image billing only when explicitly configured', async () => {
    const provider = await plugin.createProvider({
      ...
      ANTSEED_SERVICE_UNIT_BILLING_MODELS_JSON: JSON.stringify({
        'cover-art': {
          'openai-images': { version: 2, components: [{ priceMicroUsdc: "8160" }] },
        },
      }),
    });

    expect(provider.serviceUnitBillingModels?.['cover-art']?.['openai-images']).toEqual({ version: 2, components: [{ priceMicroUsdc: "8160" }] });
  });
