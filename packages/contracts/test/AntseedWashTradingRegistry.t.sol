...
contract AntseedWashTradingRegistryTest is Test {
    ...
    function test_distinctClaimIdsDoNotAddWashOrSettledVolumes() public {
        ...
        bytes memory second = _journal(bytes32(uint256(2)), SELLER, 4, 20, 1, BLOCK_NUMBER, BLOCK_HASH);
        registry.submit(second, "");

        _assertRecord(registry, 3, 10, 3_000);
        assertEq(registry.claimJournalDigest(bytes32(uint256(2))), sha256(second));
    }
