...
and commits a sorted inactivity snapshot against canonical live staking and
channel state. Sellers must be more than 14 days inactive to enter that
snapshot; exactly 14 days is rejected. P0 and submitted inactive sellers remain
permanently blocked, while unstaked and never-settled sellers fail closed on
live state. Later settlements cannot clear a submitted inactivity block. Locked
rewards remain owned by the seller and are never confiscated.

Deploy the gate with `INACTIVE_SELLERS_FILE` pointing to the sorted
`antseed-permanent-inactive-seller-snapshot` JSON generated by the forensic
reward-gate simulator. The constructor re-reads each submitted seller's live
staking and channel state and reverts unless the seller is still more than 14
days inactive. Once deployment succeeds, the stored inactivity timestamps are
immutable and later settlements cannot restore eligibility.
