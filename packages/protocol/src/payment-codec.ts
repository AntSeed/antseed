...
import { validateUnitBillingUsageReport } from './billing.js';
...
export function decodeNeedAuth(data: Uint8Array): NeedAuthPayload {
  ...
  if (obj.billingUsage !== undefined) {
    ...
    const billingUsage = obj.billingUsage as UnitBillingUsageReport;
    const errors = validateUnitBillingUsageReport(billingUsage);
    if (errors.length > 0) {
