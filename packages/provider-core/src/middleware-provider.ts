...
/**
 * Redact all occurrences of middleware phrases found in text.
...
function redactText(text: string, phrases: string[]): string {
  ...
  for (const phrase of phrases) {
    let idx = result.toLowerCase().indexOf(phrase);
    while (idx !== -1) {
      result = result.slice(0, idx) + '[content redacted]' + result.slice(idx + phrase.length);
      idx = result.toLowerCase().indexOf(phrase, idx + '[content redacted]'.length);
    }
  }
  return result;
}
...
export class MiddlewareProvider implements Provider {
  ...
  async handleRequest(req: SerializedHttpRequest): Promise<SerializedHttpResponse> {
    ...
    const resp = await this._inner.handleRequest(augmented);
  ...
  private _augment(req: SerializedHttpRequest): { req: SerializedHttpRequest; applied: boolean } {
    if (!this._middleware.length) return { req, applied: false };
    let body: Record<string, unknown>;
    try {
      body = JSON.parse(new TextDecoder().decode(req.body)) as Record<string, unknown>;
    } catch {
      return { req, applied: false }; // not JSON — leave unchanged
    }
    ...
    return {
      req: { ...req, body: new TextEncoder().encode(JSON.stringify(augmented)) },
      applied: true,
    };
  }
   ...
   * Only runs when middleware was actually applied to the request.
   */
  private _stripResponse(resp: SerializedHttpResponse, applied: boolean): SerializedHttpResponse {
    if (!resp.body || !applied) return resp;
    try {
