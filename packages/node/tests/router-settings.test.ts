...
describe('plugin-owned router settings', () => {
  ...
  it('preserves independent plugin and instance namespaces', () => {
    const input = { 'plugin:example': { policy: 'fast' }, 'instance:premium': { policy: 'quality' } };
    expect(readRouterSettings(input)).toEqual(input);
    expect(readRouterSettings(input)['plugin:example']).not.toBe(input['plugin:example']);
  });
  ...
  it('bounds namespace and field counts without mutating other instances', () => {
    ...
    expect(() => readRouterSettings({ 'plugin:example': Object.fromEntries(Array.from({ length: 51 }, (_, index) => [`field${index}`, 'value'])) })).toThrow();
