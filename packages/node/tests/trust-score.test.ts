...
describe('computeTrustScore', () => {
  ...
  it('gives established sellers up to 60 points for settled service history', () => {
    ...
    expect(trustScore(chainPeer({ onChainChannelCount: 9, onChainTotalVolumeUsdcMicros: 0 }), NOW)).toBeCloseTo(60 * 0.499 / 2, 1);
    expect(trustScore(chainPeer({ onChainChannelCount: 0, onChainTotalVolumeUsdcMicros: 9_000_000 }), NOW)).toBeCloseTo(60 * 0.499 / 2, 1);
  });
  ...
  it('lets an established seller clear the default gate while usage is bootstrapping', () => {
    ...
    expect(trustScore(seller, NOW)).toBeGreaterThanOrEqual(60);
  });
  ...
  it('adds weighted last-epoch usage share and current-epoch power share', () => {
    ...
    expect(trustScore(balanced, NOW)).toBeCloseTo(20 * shareCurve(1_000));
    const powerOnly = chainPeer({ onChainPoolPowerShareBps: 1_000 });
    expect(trustScore(powerOnly, NOW)).toBeCloseTo(5 * shareCurve(1_000));
    expect(trustScore(chainPeer(), NOW)).toBe(0);
    expect(trustScore(chainPeer({ onChainUsageShareBps: 10_000, onChainPoolPowerShareBps: 10_000 }), NOW)).toBe(20);
  });
  ...
  it('adds up to 20 points for a verified identity on top of the on-chain parts', () => {
    ...
    expect(trustScore(typical, NOW)).toBeCloseTo(20 + 20 * shareCurve(1_000));
  ...
  it('ignores ghost count, stake amount and the local sybil heuristic', () => {
    ...
    expect(trustScore(peer, NOW)).toBeCloseTo(20 * shareCurve(1_000));
  });
