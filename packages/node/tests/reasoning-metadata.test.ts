...
describe('reasoning effort announcements', () => {
  it.each([12, 13])('preserves signed v1 unit billing on metadata v%s', (version) => {
    const source = metadata(version === 13 ? { reasoningEfforts: [] } : { reasoning: true });
    source.version = version;
    source.providers[0]!.serviceApiProtocols = { model: ['openai-images'] };
    source.providers[0]!.serviceUnitBillingModels = {
      model: {
        'openai-images': {
          version: 1,
          components: [{ unit: 'output_images', priceUsd: 0.125, match: { size: '1024x1024' } }],
        },
      },
    };
