...
import { QUANTITY_BILLING_METADATA_VERSION } from "./peer-metadata.js";
...
function encodeBody(metadata: PeerMetadata): Uint8Array {
 ...
 const hasServiceCategoryExtensions = metadata.version >= SERVICE_CATEGORIES_METADATA_VERSION;
  ...
  for (const p of metadata.providers) {
    if (metadata.version < QUANTITY_BILLING_METADATA_VERSION && Object.values(p.serviceUnitBillingModels ?? {}).some(models => Object.keys(models).length > 0)) throw new Error(`Quantity billing requires metadata v${QUANTITY_BILLING_METADATA_VERSION} or newer`);
