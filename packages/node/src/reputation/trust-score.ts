 ...
 *
 * - usage (40):    the seller pool's share of all pools' recognized-usage points
 ...
 * - power (20):    the pool's share of all pools' lock-weighted staking power in
 *                  the current epoch (`AntseedSellerPools`): what a buyer's spend
 *                  with this seller earns this week.
 * - identity (40): public history of a verified identity (GitHub portfolio or
...
export const TRUST_WEIGHTS = { usage: 40, power: 20, identity: 40 } as const;
...
export interface TrustBreakdown {
  ...
  score: number;
  /** Last epoch's usage-points share, weighted 0-40; `null` when usage accounting data is unavailable. */
  usage: { score: number; shareBps: number; epoch: number } | null;
  /** Current epoch's staking-power share, weighted 0-20; `null` when pool data is unavailable. */
  power: { score: number; shareBps: number; epoch: number } | null;
  /** Verified-identity part, weighted 0-40; `null` when no verified identity has usable history. */
...
export function computeTrustScore(peer: PeerInfo, nowMs = Date.now()): TrustBreakdown | null {
  ...
  const score = washFlagged ? 0 : Math.min(100, (usage?.score ?? 0) + (power?.score ?? 0) + (identity?.score ?? 0));
