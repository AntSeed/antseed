...
import { resolveCompletedRequestPrice, type ServiceBillingOffer } from '@antseed/protocol/service-billing';
...
export function completedRequestOffer(provider: Provider, service: string): ServiceBillingOffer | undefined {
  ...
  const price = resolveCompletedRequestPrice(billingModels);
  ...
  return { provider: provider.name, service, ...price };
}
