...
// ── Test 9: /stats/buyer/:address ─────────────────────────────────────────────

describe('createServer — /stats/buyer/:address', () => {
  const PORT = nextPort();
  const BUYER = '0x' + 'b'.repeat(40);
  const SELLER_ADDR = '0x' + 'a'.repeat(40);
  ...
  before(async () => {
    ...
    store.applyBatch(
      ...
      [
        makeEvent({
          blockNumber: 100, agentId: 7n, buyer: BUYER, inputTokens: 500n,
          outputTokens: 800n, requestCount: 10n,
        }),
        makeEvent({
          blockNumber: 101, agentId: 7n, buyer: BUYER, inputTokens: 200n,
          outputTokens: 300n, requestCount: 5n,
          channelId: '0x' + '9'.repeat(64),
        }),
      ],
      101,
    );

    const peers = [fakePeer('1', SELLER_ADDR)];
    const poller = makePoller(peers);
    const stakingClient = makeStakingClient((addr) =>
      addr === SELLER_ADDR ? 7 : 0,
    );
  ...
  it('rejects malformed addresses with 400', async () => {
    const res = await fetch(`http://localhost:${PORT}/stats/buyer/not-an-address`);
    assert.equal(res.status, 400);
  });

  it('returns totals:null + bySeller:[] for unknown buyer', async () => {
    const res = await fetch(`http://localhost:${PORT}/stats/buyer/0x${'1'.repeat(40)}`);
    assert.equal(res.status, 200);
    const body = await res.json() as { totals: unknown; bySeller: unknown[] };
    assert.equal(body.totals, null);
    assert.deepEqual(body.bySeller, []);
  });

  it('returns aggregated totals + per-seller breakdown for known buyer', async () => {
    const res = await fetch(`http://localhost:${PORT}/stats/buyer/${BUYER}`);
    assert.equal(res.status, 200);
    const body = await res.json() as {
      totals: { totalRequests: string; totalInputTokens: string; totalOutputTokens: string; totalSettlements: number; uniqueSellers: number } | null;
      bySeller: Array<{ agentId: number; peerId: string | null; totalRequests: string }>;
    };

    assert.ok(body.totals);
    assert.equal(body.totals.totalRequests, '15');   // 10+5
    assert.equal(body.totals.totalInputTokens, '700');
    assert.equal(body.totals.totalOutputTokens, '1100');
    assert.equal(body.totals.totalSettlements, 2);
    assert.equal(body.totals.uniqueSellers, 1);

    assert.equal(body.bySeller.length, 1);
    assert.equal(body.bySeller[0]!.agentId, 7);
    assert.equal(body.bySeller[0]!.peerId, SELLER_ADDR.replace(/^0x/, ''));
    assert.equal(body.bySeller[0]!.totalRequests, '15');
  });

  it('returns 503 when store is not configured', async () => {
    const PORT2 = nextPort();
    const noStore = createServer({
      poller: makePoller([]),
      port: PORT2,
    });
    await noStore.start();
    const res = await fetch(`http://localhost:${PORT2}/stats/buyer/0x${'2'.repeat(40)}`);
