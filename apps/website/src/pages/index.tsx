...
const FAQ_DATA = [
  ...
  {q:'What happens when LLMs become so good that anyone can do anything?', a:"That is exactly what we want. When LLMs get dramatically more capable, costs collapse — better architectures are more efficient, open source catches up fast, and more people can run their own capable LLMs. Those people become AntSeed providers. The supply side grows, not shrinks.<br/><br/>But 'anyone can do anything' does not mean everyone delivers the same result. The value is in what you build on top — skills, workflows, domain expertise, agent orchestration. That work is custom, specialized, and endless. There will always be competition, and a massive amount of specialized work to package, sell, and improve.<br/><br/>Meanwhile, AntSeed's infrastructure becomes more valuable, not less. The protocol is purpose-built for peer-to-peer AI communication. As agents multiply and start hiring other agents, they need exactly this — an open market to find services, evaluate providers, and pay autonomously. AntSeed is the native infrastructure for an economy of intelligent machines, and it grows with every new agent the market produces."},
];
