...
**Settlement.** Buyers pre-deposit USDC into escrow. Each session is authorized by an EIP-712 SpendingAuth. Settlement is lazy — the buyer's next SpendingAuth proves delivery of the previous session and triggers on-chain settlement atomically.
