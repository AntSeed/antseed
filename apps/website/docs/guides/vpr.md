...
Unavailable checks retry with exponential backoff starting at 30 seconds, capped at five minutes. Sellers not being viewed receive at most three unavailable-check attempts per app/buyer session or observed capability change; returning to their model resumes retries. Automatic checks never select or pin sellers, install plugins, or change verification policy. A passing seller-node check **does not guarantee that downstream inference runs inside a TEE**; provider, GPU, and measured-image claims are not enforced by this setting.

In **Preferences**, **Require seller-node verification** is a separate routing policy, off by default. When enabled, both required seller-node claims must pass before inference or payment negotiation, including pinned routes. Auto routing can try another eligible seller; a failing pin returns an error without changing the pin. The browse filter remains independent.

Changing this policy requires confirmation and restarts the owned buyer, interrupting active requests. Connected apps keep their proxy settings; forwarding resumes only after the new buyer confirms the requested policy. On failure the buyer remains stopped, without silently restoring optional mode. Use **Reapply saved setting** to retry. Offline changes apply at the next start. A named development instance or a window attached to a shared buyer cannot restart the shared buyer: change the policy in its owning runtime instead. The UI shows configured versus active policies when CLI flags override the saved setting.
