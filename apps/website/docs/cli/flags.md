...
antseed config seller add-service <provider> <service> \
  --image-edit-model '<venice-edit-model>' \
  --capabilities '<json>' \
  --unit-billing-models '<json>'
```

Capabilities and unit billing can also be entered in `antseed seller setup`. Native Venice edit pairings can be written directly as `imageEditModel` under `seller.providers.<provider>.services.<service>` in `config.json`. Use `match.operation` values `image_generation` and `image_edit` in `--unit-billing-models` when those operations have different prices.
