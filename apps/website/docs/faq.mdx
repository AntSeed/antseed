...
export const faqLd = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'Standard providers receive the request content needed to provide the service and may be able to inspect or log it. A correctly verified TEE route can reduce operator visibility, but the attestation must be checked against the expected workload, seller identity, and configuration. Availability varies by seller and model.',
      },
    },
    {
      '@type': 'Question',
...
**Layer 2 — TEE-verified providers.** These run inside Trusted Execution Environments such as Intel TDX or AMD SEV-SNP. A correctly verified route can reduce what the node operator can see, but users must verify that the attestation binds the expected workload, seller identity, and configuration. Availability varies by seller and model. See [Verify a seller's TEE](/docs/guides/verify-tee).

Peer traffic uses mutually authenticated encrypted TCP where supported. WebRTC DataChannels are a fallback for peers that cannot use direct encrypted TCP; WebRTC is not the primary node-to-node transport. See [Transport](/docs/transport).
...
Standard providers receive the request content needed to provide the service and may be able to inspect or log it. A correctly verified TEE route can reduce operator visibility, but a TEE label alone is not sufficient proof. Verify that the attestation matches the expected workload, seller identity, and configuration. Availability varies by seller and model.
