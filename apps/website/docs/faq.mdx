...
No account required. Install the CLI, run `antseed buyer start`, and request any model listed by `curl localhost:8377/v1/models`. The proxy applies the same Price + Trust preferences as the desktop AI VPN, including the default hard minimum trust score of `60`. Pinning a specific seller with `antseed buyer connection set --peer <peerId>`, `x-antseed-pin-peer`, or `<peerId>@<model>` remains available as an explicit override. Tools that speak a supported OpenAI or Anthropic API format work immediately.
