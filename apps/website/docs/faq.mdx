...
export const faqLd = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'No account required. Install the CLI, start the buyer proxy, and you are live on the network — request any listed model and the proxy auto-selects the best peer serving it. Any tool that speaks the OpenAI API format works immediately.',
      },
    },
    {
      '@type': 'Question',
...
No account required. Install the CLI, run `antseed buyer start`, and you are live on the network — request any model listed by `curl localhost:8377/v1/models` and the proxy auto-selects the highest-reputation peer serving it (pinning a specific peer with `antseed buyer connection set --peer <peerId>` stays available as an override). Any tool that speaks the OpenAI API format works immediately.
