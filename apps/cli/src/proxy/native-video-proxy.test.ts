...
test('Veo download URLs point to the local proxy without changing the signed upstream response', () => {
  ...
  const rewritten = rewriteVideoDownloadUrls(route, upstream, 'http://127.0.0.1:8377', 'veo-stream-v1')
  ...
  assert.equal(rewriteVideoDownloadUrls(route, hosted, 'http://127.0.0.1:8377', 'veo-stream-v1'), hosted)
