...
test('verifierRewardSource binds verifier address and signer to both contracts', async () => {
  const calls: unknown[][] = []
  ...
  const source = verifierRewardSource(
    ...
    {
      epochRewardClaimed: async (...args: unknown[]) => { calls.push(['claimed', ...args]); return false },
