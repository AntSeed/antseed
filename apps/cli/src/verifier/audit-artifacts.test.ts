...
test('verifier artifacts use epoch and model directories', async () => {
  ...
  try {
    ...
    assert.match(reportPath, /epochs\/7\/runs\/run\/report\.html$/)
    assert.match(report, /^<!doctype html>/)
    assert.match(report, /Seller A \(111111111111…\)/)
    assert.match(report, /<td class="numeric">10\/10<\/td>/)
    assert.match(report, /<td class="numeric">90\.0%<\/td>/)
    assert.match(report, /<span class="badge same">SAME<\/span>/)
    assert.match(report, /Skipped Seller \(222222222222…\)/)
    assert.match(report, /<span class="badge skipped">SKIPPED<\/span>/)
    assert.match(report, /price_above_maximum: price exceeds buyer maximum/)
    assert.match(report, /adjust buyer routing policy/)
    assert.match(report, /<code>price_above_maximum<\/code>: 1/)
    assert.match(report, /href="file:\/\/\/tmp\/audit\.json"/)
...
test('latest epoch report merges repair outcomes without changing immutable run reports', async () => {
  ...
  try {
    ...
    assert.match(latest.reportPath, /epochs\/9\/report\.html$/)
    assert.match(latestReport, /consolidated latest epoch snapshot/)
    assert.match(latestReport, /Seller A/)
    assert.match(latestReport, /Seller B/)
    assert.doesNotMatch(latestReport, /deadline_exceeded/)
    assert.match(latestReport, /Cumulative estimated cost/)
    assert.match(latestReport, /\$1\.250000/)
